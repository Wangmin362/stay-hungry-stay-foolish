...
/* (思路一) 面试官大牛前辈您好，我的设计思路如下：（假设可以使用开源项目，我这里会选择使用yamux多路复用器）
...
/* (思路二)  由于您这边对于连接的使用已经明确规定了只能使用go sdk，因此只能自己来实现底层操作。但是yamux的思路是行得通的。在数据传输过程中，
由于TCP的字节流特性，因此应用层无法区分每个包的边界，所以这里需要区分当前传输的是key还是数据，以及还需要区分当前传输数据是哪个key的数据。
做完以上分析之后，我大概有了以下简单的设计。
	// 以下是对于key的数据包定义
		++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
